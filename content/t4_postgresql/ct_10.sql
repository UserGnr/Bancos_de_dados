...
SELECT AVG(preco_venda) FROM produtos; -- possui precisão grande

SELECT TO_CHAR(AVG(preco_venda),'99999999D99') AS Media FROM produtos; -- Formatado para string

SELECT ROUND(AVG(preco_venda)::numeric,2) FROM produtos; -- Formatado para númerico

SELECT ROUND(AVG(preco_custo::numeric), 2) FROM produtos; -- Calculando a média de um valor Money
